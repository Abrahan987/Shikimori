...
shikimori Bot es un bot de WhatsApp multifuncional basado en `baileys`. Este bot ofrece una variedad de características para mejorar tu experiencia en WhatsApp.
...
```
```bash
git clone https://github.com/The-King-Destroy/shikimoriBot-MD && cd shikimoriBot-MD
```
```bash
yarn install
```
...
> *Ejecutar estos comandos dentro de la carpeta shikimoriBot-MD*
```bash
...
```bash
cd && cd shikimoriBot-MD && npm start
```
----
### Obtener nuevo código QR
> *Detén el bot, haz click en el símbolo (ctrl) [default=z] usar la letra "z" + "ENTER" hasta que salga algo verdes similar a: `shikimoriBot-MD $`*
> **Escribe los siguientes comandos uno x uno :**
```bash
cd && cd shikimoriBot-MD && rm -rf sessions/Principal && npm run qr
```
----
### Obtener nuevo código de teléfono
```bash
cd && cd shikimoriBot-MD && rm -rf sessions/Principal && npm run code
```
</details>

<details>
<summary><b>❀ Actualizar shikimoriBot-MD</b></summary>

> **Utiliza esta opción únicamente si deseas actualizar a la última versión de shikimoriBot. Hemos implementado un método ingenioso mediante comandos para realizar la actualización, pero ten en cuenta que al usarla se eliminarán todos los archivos de la versión actual y se reemplazarán con los de la nueva versión. Solo se conservará la base de datos, por lo que será necesario volver a vincular el Bot.**

**Comandos para actualizar shikimoriBot-MD de forma automática**

```bash
grep -q 'bash\|wget' <(dpkg -l) || apt install -y bash wget && wget -O - https://raw.githubusercontent.com/The-King-Destroy/shikimoriBot-MD/master/termux.sh | bash
```
...
```bash
cd && cd shikimoriBot-MD && nano settings.js
```
#### Para que no pierda su progreso en shikimoriBot, estos comandos realizarán un respaldo de su `database.json` y se agregará a la versión más reciente.
...
### **`❀ Colaboradores`**
<a href="https://github.com/The-King-Destroy/shikimoriBot-MD/graphs/contributors">
<img src="https://contrib.rocks/image?repo=The-King-Destroy/shikimoriBot-MD" />
</a>
